    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                ...
                const tableResponse = await axios.get(`https://barhotelbackend.vercel.app/api/table/tables/${tableId}`);
                const table = tableResponse.data;

                // Fetch the section information using the section ID
                const sectionResponse = await axios.get(`https://barhotelbackend.vercel.app/api/section/${table.section._id}`);
