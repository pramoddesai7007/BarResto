...
const EditOrderPage = ({ params, tableId }) => {
      ...
      try {
        if (orderNumber) {
          ...
          const orderResponse = await axios.get(
            `https://barhotelbackend.vercel.app/api/order/get/order/${orderNumber}`
          );
          ...
          const sectionResponse = await axios.get(
            `https://barhotelbackend.vercel.app/api/section/sectionlist/${tableId}`
          );
  ...
  const handleSaveBill = async () => {
    try {
      ...
      await axios.post(`https://barhotelbackend.vercel.app/api/logHistory/log-history`, {
      ...
      const tableResponse = await axios.get(
        `https://barhotelbackend.vercel.app/api/table/tables/${tableId}`
      );
      const tableInfo = tableResponse.data;

      // Construct the API endpoint for updating the order based on order number
      const updateOrderEndpoint = `https://barhotelbackend.vercel.app/api/order/update-order-by-number/${orderNumber}`;
...
`;
  ...
  useEffect(() => {
    axios
      .get("https://barhotelbackend.vercel.app/api/main")
      .then((response) => {
        console.log(response.data);
          ...
          <div className=" w-full lg:w-2/5 bg-gray-100 md:w-96 lg:-mt-3 -mt-2 md:-mt-1">
            ...
            <div className="flex px-5 mt-2 justify-between">
              <div className=" sm:w-auto mb-2 sm:mb-0 sm:mr-2 ">
                <div
                  className="px-3 py-2 rounded-md text-center bg-green-700 text-white font-bold cursor-pointer text-xs"
