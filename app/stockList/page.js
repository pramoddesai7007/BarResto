...
const StockOutwardTable = () => {
  ...
  const fetchStockOutwardList = async (start, end) => {
    try {
      const response = await axios.get('https://barhotelbackend.vercel.app/api/stockOut/stockOut', {
        params: {
          startDate: start,
          endDate: end,
        },
  ...
  const fetchItems = async () => {
    try {
      ...
      const itemsWithPrice = await Promise.all(response.data.map(async (item) => {
        const purchaseBillsResponse = await axios.get('https://barhotelbackend.vercel.app/api/purchase/purchases', {
          params: {
