...
const Section = () => {
  ...
  useEffect(() => {
    ...
    // Fetch default section name from the server and store it in local storage
    const fetchDefaultSectionName = async () => {
      try {
        const response = await axios.get("https://barhotelbackend.vercel.app/api/section/section/default");
        const defaultSectionName = response.data.name;
        localStorage.setItem('defaultSectionName', defaultSectionName);
      } catch (error) {
        console.error("Error fetching default section name:", error);
      }
    };
  
    fetchDefaultSectionName();
  }, []);
  ...
  useEffect(() => {
    const fetchSections = async () => {
      try {
        const response = await axios.get("https://barhotelbackend.vercel.app/api/section");
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      await axios.patch(
        `https://barhotelbackend.vercel.app/api/section/${sectionToEdit._id}`,
        {
    ...
    if (sectionToEdit.isDefault) {
      localStorage.setItem('defaultSectionName', sectionToEdit.name);
    } else {
      // Clear default section name from local storage if it's not default
      localStorage.removeItem('defaultSectionName');
    }
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      await axios.delete(
        `https://barhotelbackend.vercel.app/api/section/${sectionToDelete._id}`
      );
      ...
      const sectionsResponse = await axios.get(
        "https://barhotelbackend.vercel.app/api/section"
      );
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "https://barhotelbackend.vercel.app/api/section/create",
        formData,
        {
          headers: {
