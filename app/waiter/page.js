...
const Waiter = () => {
  ...
  const handleAddWaiter = async () => {
    ...
    if (waiterData.contactNumber.length !== 10 || isNaN(waiterData.contactNumber)) {
      ...
      const response = await axios.post('https://barhotelbackend.vercel.app/api/waiter', waiterData);
  ...
  //   try {
  //     const response = await axios.put(`https://barhotelbackend.vercel.app/api/waiter/${waiterId}`, waiterData);
  ...
  const confirmDeleteWaiter = async () => {
    try {
      await axios.delete(`https://barhotelbackend.vercel.app/api/waiter/${deletingWaiterId}`);
