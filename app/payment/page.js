...
const PaymentModal = ({
  ...
  const fetchMobileNumbers = async () => {
    try {
      const response = await axios.get(
        "https://barhotelbackend.vercel.app/api/customer/mobile-numbers"
      );
    ...
    try {
      ...
      if (formData.mobileNumber) {
        const customerResponse = await axios.get(
          `https://barhotelbackend.vercel.app/api/customer/get-customer-by-mobile/${formData.mobileNumber}`
        );
      ...
      if (formData.mobileNumber && parseFloat(formData.dueAmount) > 0) {
        ...
        if (updatedCreditBalance > 0) {
          await axios.patch(
            `https://barhotelbackend.vercel.app/api/customer/update-credit-balance/${existingCustomer._id}`,
            {
      ...
      const response = await axios.patch(
        `https://barhotelbackend.vercel.app/api/order/update-order-by-id/${orderID}`,
        {
          ...formData,
      ...
      // const tableId = response.data.tableId; // Assuming response contains tableId
      // await axios.put(`https://barhotelbackend.vercel.app/api/table/${tableId}`, {
      //   isCalled: false,
      // });
  ...
  const addNewCustomer = async () => {
    ...
    const newCustomerResponse = await axios.post(
      "https://barhotelbackend.vercel.app/api/customer/customer/list/add-customer",
      {
        customerName: formData.customerName,
        mobileNumber: formData.mobileNumber,
  ...
  const fetchCustomerDetails = async (mobileNumber) => {
    try {
      const response = await axios.get(
        `https://barhotelbackend.vercel.app/api/customer/get-customer-by-mobile/${mobileNumber}`
      );
  ...
  const handlePaymentMethodClick = async () => {
    try {
      ...
      await axios.put(`https://barhotelbackend.vercel.app/api/order/order/update-flag/${orderNumber}`);
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left  shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full ">
          <div className="container mx-auto lg:p-8 md:p-8 p-2">
            ...
            <form onSubmit={handlePayment} className="max-w-md mx-auto">
              ...
              <div>
                <h1 className="text-center font-bold mb-3" onClick={handlePaymentMethodClick} // Attach onClick event handler
                >Payment Method</h1>
