...
const CounterMainMenuList = () => {
  ...
  const handleViewMainCategory = async (mainCategory) => {
    try {
      const response = await axios.get(
        `https://barhotelbackend.vercel.app/api/main/${mainCategory._id}`
      );
  ...
  const handleDeleteConfirmed = async (menu) => {
    try {
      ...
      const response = await axios.delete(
        `https://barhotelbackend.vercel.app/api/main/${menu._id}`
      );
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const response = await axios.patch(
        `https://barhotelbackend.vercel.app/api/main/${editedMenu._id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      const updatedMenusResponse = await axios.get("https://barhotelbackend.vercel.app/api/main");
  ...
  const handlePicturePreview = (mainImage) => {
    const completeImageUrl = `https://barhotelbackend.vercel.app/${mainImage}`;
              ...
              <div className="p-1 text-center">
                ...
                <p>
                  ...
                  {mainCategoryToView?.mainImage ? (
                    <img
                      src={`https://barhotelbackend.vercel.app/${mainCategoryToView.mainImage}`}
                      alt="Main Category"
