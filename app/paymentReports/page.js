...
const PaymentList = () => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                setLoading(true);
                const response = await axios.get('https://barhotelbackend.vercel.app/api/payment/paymentsList');
                const paymentsWithTableNames = await Promise.all(
                    response.data.map(async (payment) => {
                        const tableResponse = await axios.get(`https://barhotelbackend.vercel.app/api/table/tables/${payment.tableId}`);
                        return {
                            ...payment,
