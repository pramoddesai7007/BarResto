...
const CounterGstForm = () => {
    ...
    useEffect(() => {
        const fetchHotelAndGSTPercentage = async () => {
            try {
                const hotelsResponse = await axios.get('https://barhotelbackend.vercel.app/api/counterHotel/get-all');
    ...
    useEffect(() => {
        const fetchGSTPercentage = async () => {
            try {
                if (hotel._id) {
                    const gstResponse = await axios.get(`https://barhotelbackend.vercel.app/api/counterHotel/get/${hotel._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (hotel._id && gstPercentage !== '') {
            try {
                await axios.patch(`https://barhotelbackend.vercel.app/api/counterHotel/gst/${hotel._id}`, {
    ...
    return (
        ...
        {isEmployeeAuthTokenPresent ? <Navbar /> : <CouponNavbar />}
            ...
            <span> GST Master (Counter Coupon)</span>
            </div>
